  ...
  const getClass = async () => {
    try {
      const result = await axios.get(
        `http://192.168.3.9:3000/api/classes/one/${id}`
      );
    ...
    <ScrollView>
      <View>
        <Pressable
          style={styles.btn}
          onPress={() =>
            navigation.navigate("ListaMateriales", {
              id: curso.idCurso,
            })
          }
        >
        ...
        <Pressable style={styles.btn} onPress={() => navigation.navigate('ListarEstudiantes', { id: curso.idCurso })}>
